...
void drawEllipse(int centerX, int centerY, int radiusX, int radiusY)
{
    ...
    int x = 0;
    int y = radiusY;
    int decision = pow(radiusY, 2) - pow(radiusX, 2) * radiusY + (0.25 * pow(radiusX, 2));

    glBegin(GL_POINTS);

    // Region 1
    while (2 * pow(radiusX, 2) * y >= 2 * pow(radiusY, 2) * x)
    {
    ...
    // Region 2
    decision = pow(radiusY, 2) * pow((x + 0.5), 2) + pow(radiusX, 2) * pow((y - 1), 2) - pow(radiusX, 2) * pow(radiusY, 2);
